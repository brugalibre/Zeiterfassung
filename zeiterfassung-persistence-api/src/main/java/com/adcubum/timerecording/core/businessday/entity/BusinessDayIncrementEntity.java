...
public class BusinessDayIncrementEntity extends BaseEntity {
   ...
   public <T extends BaseEntity> boolean hasChangesForLastModified(T persistentOther) {
      if (persistentOther instanceof BusinessDayIncrementEntity) {
         BusinessDayIncrementEntity persistentBusinessDayIncrementEntity = (BusinessDayIncrementEntity) persistentOther;
         return this.isBooked() != persistentBusinessDayIncrementEntity.isBooked()
                 || this.isSent() != persistentBusinessDayIncrementEntity.isSent()
                 || !this.getDescription().equals(persistentBusinessDayIncrementEntity.getDescription())
                 || !this.getServiceCode().equals(persistentBusinessDayIncrementEntity.getServiceCode())
                 || !this.getTicketNr().equals(persistentBusinessDayIncrementEntity.getTicketNr());
      }
      return false;
   }

...
/**
 * The {@link Drawable} class can be used to draw a representation of any instantiated object on the {@link MainUserInterface} component
 * @author Dominic
 * @param <E> the value this Drawable represents on the {@link MainUserInterface}
 * 
 */
public abstract class Drawable<E> implements Renderer
{
   public enum UpdateType {
      REPAINT
      , RECALCULATE
      , GRAB_FOCUS                      // after the this Drawable has done some action or before doing something next, the parent component can be forced to grab focus
      , CHECK_CONDITION                 // e.g. during the drawable is moving, the condition can be checked
      , AFTER_ACTION                    // after this Drawable is finished with actions, that should be updated at the end, instead of permanently    
   }
   ...
   protected int             x;         // the x-coordinate in relation to it's position on the TimeLine (used for painting)
   protected int             y;         // the y-coordinate in relation to it's position on the TimeLine (used for painting)
   ...
   /**
    * Sets the bounds the layout manager has assigned to this {@link Drawable}. Those, of course, have to be considered in the rendering process.
   ...
   /**
    * Forwards the specific type of {@link UpdateType} to it's {@link ParentComponent} in order it can take further stepts according the kind of update
    * @param type, kind of update to do
    */
